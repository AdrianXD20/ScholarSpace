...
export const actividadesService = {
  ...
  async createActividad(payload: CreateActividadRequest): Promise<{ ok: boolean; data?: ActividadApi; error?: string }> {
    try {
  ...
  async updateActividad(
    id: number,
    payload: Partial<CreateActividadRequest>
  ): Promise<{ ok: boolean; data?: ActividadApi; error?: string }> {
    try {
  ...
  async deleteActividad(id: number): Promise<{ ok: boolean; error?: string }> {
    try {
      await httpClient(`${endpoints.actividades.list}/${id}`, { method: 'DELETE' })
      return { ok: true }
    } catch (e) {
